...
export default function ProfileForm({ userId, profile }: { userId: string; profile?: Profile }) {
  ...
  // Step 2: cropper confirmed → upload blob
  ...
  return (
    <>
      {/* Cropper modal — full screen when active */}
      {cropSrc && (
        <AvatarCropper
          imageSrc={cropSrc}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center px-4 py-8 overflow-y-auto">
            <div className="w-full max-w-sm relative">
              <button onClick={() => setShowPreview(false)}
                className="absolute -top-10 right-0 text-subtle hover:text-pearl text-sm transition-colors">
                ✕ Cerrar
              </button>
      ...
      <form onSubmit={handleSubmit} className="surface-card p-4 md:p-6 flex flex-col gap-5">
        ...
        <div className="flex items-center justify-end">
          <button type="button" onClick={() => setShowPreview(true)}
            className="btn-ghost-gold text-xs py-1.5 px-4 flex items-center gap-1.5">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col gap-2 flex-1">
              <button type="button" onClick={() => fileRef.current?.click()} disabled={uploading} className="btn-ghost-gold text-sm py-2 w-full">
                {uploading ? 'Subiendo...' : avatarPreview ? 'Cambiar y recortar foto' : 'Subir y recortar foto'}
              </button>
              <p className="text-subtle text-xs">JPG, PNG, WEBP — recorte circular automático</p>
            </div>
          </div>
