...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const { data: account } = await supabase.from('user_accounts').select('id').eq('username', username).single()
  if (!account) return { title: 'SOY_CARD_PRO' }
  const { data: profile } = await supabase.from('profiles').select('display_name, title, company, bio, avatar_url')
...
export default async function PublicProfilePage({ params }: Props) {
  ...
  const appUrl = `${process.env.NEXT_PUBLIC_APP_URL ?? 'https://soycardpro.vercel.app'}/${username}/${slug}`
