...
const FEATURES = [
  {
    icon: '◈',
    title: 'Multi-perfil',
    desc: 'Crea perfiles distintos para cada faceta de tu vida: CEO, consultor, creador. Un solo QR lo agrupa todo.',
  },
  {
    icon: '📡',
    title: 'NFC Inteligente',
    desc: 'Comparte tu tarjeta acercando tu teléfono. Sin apps, sin fricción. Solo campo a campo.',
  },
  {
    icon: '◉',
    title: 'Captura de leads',
    desc: 'Quien escanea tu tarjeta puede dejarte su contacto en segundos. CRM integrado en tu dashboard.',
  },
  {
    icon: '◎',
    title: 'Analítica real',
    desc: 'Saber cuántas veces escanearon tu tarjeta, desde qué canal y con qué dispositivo.',
  },
  {
    icon: '⬇',
    title: 'vCard automática',
    desc: 'Con foto y redes sociales incluidas. Un toque y tu contacto queda guardado en el teléfono de quien te conoce.',
  },
  {
    icon: '◳',
    title: 'Google Wallet',
    desc: 'Tu tarjeta en el monedero digital de tus contactos. Siempre a mano, sin papel.',
  },
]

const STEPS = [
  { n: '01', title: 'Crea tu perfil', desc: 'Nombre, foto, cargo, empresa, redes sociales. Listo en 2 minutos.' },
  { n: '02', title: 'Comparte tu QR o link', desc: 'Muéstralo en pantalla, envíalo por WhatsApp o escríbelo en NFC.' },
  { n: '03', title: 'Captura contactos', desc: 'Quien lo escanea deja sus datos. Tú los ves en tiempo real en tu dashboard.' },
]
...
export default function HomePage() {
  return (
    <main style={{ background: 'var(--black-deep)', minHeight: '100vh', overflowX: 'hidden' }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-w-2xl mx-auto">
          {PLANS.map((plan, i) => (
            <div key={i} className={`p-8 rounded-2xl flex flex-col gap-5 ${plan.highlight ? 'shimmer-border' : ''}`}
              style={{
                background: plan.highlight ? 'var(--black-card)' : 'var(--black-surface)',
      ...
      {/* ── CTA FINAL ── */}
      <section className="px-6 py-24 text-center" style={{ background: 'var(--black-card)' }}>
        <div className="max-w-2xl mx-auto fade-in-up">
          <div className="divider-gold w-16 mx-auto mb-8" />
          <h2 className="text-display text-pearl text-3xl mb-4">
            Tu primer contacto te espera
          </h2>
          <p className="text-muted text-lg mb-10">
            Crea tu tarjeta digital en 2 minutos. Sin tarjeta de crédito.
          </p>
      ...
      {/* ── FOOTER ── */}
      <footer className="px-6 py-8 text-center border-t" style={{ borderColor: 'var(--black-border)' }}>
