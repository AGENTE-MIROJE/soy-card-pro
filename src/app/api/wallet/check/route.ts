...
export async function GET() {
  try {
    ...
    if (getRes.ok) {
      return NextResponse.json({ ok: true, step: 'class_exists', classId, reviewStatus: getData.reviewStatus })
    }

    // 3 — class no existe, intentar crearlo
    const createBody = {
      id: classId,
    ...
    return NextResponse.json({
      ok: createRes.ok,
      step: createRes.ok ? 'class_created' : 'class_create_failed',
      classId,
